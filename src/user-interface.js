...
projects.addEventListener("click", (e) => {
  ...
  if (e.target.matches("div") && e.target.classList.contains("project")) {
    ...
    (e.target.classList.contains("edit-project-title") ||
      e.target.classList.contains("project-title-div")) &&
    e.target.classList.contains("high-light")
  ) {
    ...
    (e.target.classList.contains("edit-project") ||
      ...
      e.target.classList.contains("cancel-project-btn"))
  ) {
    e.target.parentNode.parentNode.style.cssText = projectStyle;
    projectDivId = e.target.parentNode.parentNode.id;
    button.classList.remove("hidden");
  ...
  if (e.target.matches("span") && e.target.textContent == "check") {
    ...
    storage = localStorage.getItem("todolist");
    const storageObject = JSON.parse(storage);
    storageObject[`${input.value}**${projectId}`] =
      storageObject[storageKeyToEdit];
...
function createTask() {
  ...
  storage = localStorage.getItem("todolist");
  storage = JSON.parse(storage);
  storage[`${project.getProjectTitle()}**${project.getProjectId()}`].push([
    taskTitle,
    taskDescription,
    taskDate,
    taskPriority,
...
function updateTask() {
  ...
  const tasks =
    storage[
      `${toDoList.getProject(projectDivId).getProjectTitle()}**${toDoList
        .getProject(projectDivId)
        .getProjectId()}`
    ];
